...
class System:
    @staticmethod
    def get_files(
        parent_dir: pathlib.Path, pattern: typing.List[str]
    ) -> typing.Generator:
        """
    ...
    @staticmethod
    def get_files_recursion(
        dpath: str, pattern: typing.List[str], depth: int = 0
    ) -> typing.Generator:
        """
        :param dpath:
        :param pattern:
    ...
    def open_file_using_thread(
        cmdpath: pathlib.Path,
        filepath: typing.Union[pathlib.Path, None],
        ...
        if not cmdpath.exists():
            ...
            return False
        t = threading.Thread(
            target=System.open_file_with_arguments, args=(cmdpath, filepath, with_term)
        )
        t.daemon = True
        t.start()
        return True
    ...
    def open_file_with_arguments(
        ...
        if exitcode != 0:
            sys.stderr.write(
                "{0}, {1}, {2}".format(exitcode, out.decode("utf8"), err.decode("utf8"))
            )
            return 127
        return exitcode
